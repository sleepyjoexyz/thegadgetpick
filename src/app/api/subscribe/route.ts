...
async function sendWelcomeEmail(email: string, siteName: string): Promise<boolean> {
  ...
  const siteConfig: { [key: string]: { title: string; description: string } } = {
    thegadgetpick: {
      title: "The Gadget Pick",
      description: "Get the best tech and gadget deals delivered to your inbox.",
    },
  };

  const config = siteConfig[siteName] || { title: "The Gadget Pick", description: "Get the best deals delivered to your inbox." };
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Send welcome email
    const emailSent = await sendWelcomeEmail(trimmedEmail, "thegadgetpick");
